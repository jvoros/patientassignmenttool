...
describe("Zone Controller", () => {
  ...
  describe("Join Zones", () => {
    it("should only add shifts once", () => {
      const zoneList = Zone.make(dummy.zones.off);
    ...
    it("Simple Zones: should add at start", () => {
      const zoneSimple = Zone.make(dummy.zones.ft);
    ...
    it("Rotation Zone: should add at next pointer", () => {
      const zoneRotation = Zone.make(dummy.zones.main);
    ...
    it("Dual Zone: should add at next pointer", () => {
      const zoneDual = Zone.make(dummy.zones.main);
    ...
    it("Dual Zone: should keep super on same shift if added shift is before super", () => {
      const zoneDual = Zone.make(dummy.zones.main);
      Zone.joinZone({ zone: zoneDual, shift: docShift1 });
      Zone.joinZone({ zone: zoneDual, shift: appShift1 });
      Zone.joinZone({ zone: zoneDual, shift: docShift2 });
      expect(zoneDual.super).toEqual(2);
    });
  });
  ...
  describe("Move Pointer", () => {
    it.todo(
      "should only move pointer for zones that have that kind of pointer",
    );

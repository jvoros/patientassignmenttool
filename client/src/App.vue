...
<template>
  ...
  <div v-if="store.user.loggedIn" class="relative">
    ...
    <div class="grid grid-cols-1 md:grid-cols-10 gap-10 px-4 my-8">
      ...
      <div class="bg-gray-50 md:col-span-3 order-2 md:order-2">
        <BoardRotation :rotation="store.board.rotations.ft" />
        <BoardRotation :rotation="store.board.rotations.off" />
      </div>
    </div>
  </div>
</template>

...
<template>
  <Popover class="relative text-base" v-slot="{ close }">
    <PopoverButton class="focus:outline-none">
      <Button v-if="variety === 'default'" class="w-30" leftIcon="walk-in" />
      <Button
        v-if="variety === 'next'"
        class="w-30"
        variety="contrast"
    ...
    <PopoverTransition>
      <PopoverPanel
        class="absolute z-10 top-12 right-0 p-4 bg-gray-100 shadow-xl rounded-md border-2 border-gray-300"
      >
        <div class="flex flex-col gap-y-4 text-md">
          <ul class="flex border border-gray-300 divide-x rounded-lg">
            <li v-for="x in assign_types" class="relative grow typeButton">
              ...
              <label
                :for="x.value"
                class="cursor-pointer px-6 py-3 flex hover:bg-gray-200"
                :class="[x.value === pt_type ? 'bg-gray-200' : '']"
              >
                <Icon :icon="x.value" />
              </label>
              <span
                class="absolute mt-2 invisible whitespace-nowrap text-white bg-gray-500 px-2 py-1 rounded text-center"
                >{{ x.display }}</span
              >
            </li>
          </ul>

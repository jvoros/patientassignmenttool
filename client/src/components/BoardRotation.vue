...
<template>
  <BoardPanel :header="rotation.name + ' Rotation'">
    <div v-for="shift, index in rotation.shifts"  
      class="my-6 rounded-md hover:bg-gray-50 border" 
      :class="[getStyles(), isNext(index) ? 'shadow-md border-2 !border-amber-200' : '']"
    >
      <!-- flex: shift controls -->
      <div class="flex py-2 px-4 rounded-t items-center p-1 bg-gray-100" :class="[pointer ? 'justify-between' : 'justify-end']">
        <div v-if="pointer">
